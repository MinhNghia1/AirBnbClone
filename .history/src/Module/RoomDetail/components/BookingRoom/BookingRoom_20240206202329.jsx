...
export default function BookingRoom({
  ...
  const isBookingValid = () => {
    const isRoomAlreadyBooked = ListOfRoomIsBooked.some((booking) => {
      const startDateMatch = dayjs(booking.ngayDen).isSame(
        dayjs(selectionRange.startDate),
        "day"
      );
      const endDateMatch = dayjs(booking.ngayDi).isSame(
      ...
      if (startDateMatch && endDateMatch && sameCount && sameUser) {
        setErrorMessage("Phòng đã được đặt vào ngày này bởi bạn.");
      }

      return startDateMatch && endDateMatch && sameCount && sameUser;
    });
