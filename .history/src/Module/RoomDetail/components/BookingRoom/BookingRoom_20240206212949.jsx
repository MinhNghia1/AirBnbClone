...
export default function BookingRoom({
  ...
  const isBookingValid = () => {
    const isRoomAlreadyBooked = ListOfRoomIsBooked.some((booking) => {
      ...
      const isSelectionWithinBookingRange = selectionStartDate.isSameOrAfter(bookingStartDate, 'day') && selectionStartDate.isSameOrBefore(bookingEndDate, 'day');
      const isSelectionEndWithinBookingRange = selectionEndDate.isSameOrAfter(bookingStartDate, 'day') && selectionEndDate.isSameOrBefore(bookingEndDate, 'day');
  
      const isBookingWithinSelectionRange = bookingStartDate.isSameOrAfter(selectionStartDate, 'day') && bookingStartDate.isSameOrBefore(selectionEndDate, 'day');
      const isBookingEndWithinSelectionRange = bookingEndDate.isSameOrAfter(selectionStartDate, 'day') && bookingEndDate.isSameOrBefore(selectionEndDate, 'day');
  
      const isBookingStartBeforeSelectionStart = bookingStartDate.isBefore(selectionStartDate, 'day');
      const isBookingEndAfterSelectionEnd = bookingEndDate.isAfter(selectionEndDate, 'day');
  
      const isBookingInRange = (
        (isSelectionWithinBookingRange && isSelectionEndWithinBookingRange) ||
        (isBookingWithinSelectionRange && isBookingEndWithinSelectionRange) ||
        (isBookingStartBeforeSelectionStart && isBookingEndAfterSelectionEnd)
      );
  
      const sameUser = booking.maNguoiDung === currentUser.user.id;
      const sameCount = booking.soLuongKhach === count;
  
      return isBookingInRange && sameUser && sameCount;
    });
