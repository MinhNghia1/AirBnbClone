...
export default function BookingRoom({
  ...
  const isBookingValid = () => {
    const isRoomAlreadyBooked = ListOfRoomIsBooked.some((booking) => {
      ...
      // Check if the selection range overlaps with any existing booking range
      const isOverlap =
        (selectionStartDate.isSame(bookingStartDate, 'day') || selectionStartDate.isAfter(bookingStartDate, 'day')) &&
        (selectionStartDate.isBefore(bookingEndDate, 'day') || selectionStartDate.isSame(bookingEndDate, 'day')) ||
        (selectionEndDate.isAfter(bookingStartDate, 'day') || selectionEndDate.isSame(bookingStartDate, 'day')) &&
        (selectionEndDate.isSameOrBefore(bookingEndDate, 'day') || selectionEndDate.isBefore(bookingEndDate, 'day')) ||
        (bookingStartDate.isSame(selectionStartDate, 'day') || bookingStartDate.isAfter(selectionStartDate, 'day')) &&
        (bookingStartDate.isBefore(selectionEndDate, 'day') || bookingStartDate.isSame(selectionEndDate, 'day')) ||
        (bookingEndDate.isAfter(selectionStartDate, 'day') || bookingEndDate.isSame(selectionStartDate, 'day')) &&
        (bookingEndDate.isSameOrBefore(selectionEndDate, 'day') || bookingEndDate.isBefore(selectionEndDate, 'day'));
  ...
  const disabledRanges = ListOfRoomIsBooked
    .filter(booking => booking.maPhong === roomId)
    .map((booking) => {
      return { ngayDen: booking.ngayDen, ngayDi: booking.ngayDi };
    });
