...
export default function BookingRoom({
  ...
  console.log(ListOfRoomIsBooked, "ListOfRoomIsBooked");
...
const disabledRangesFiltered = ListOfRoomIsBooked.filter((booking) => {
  const bookingStartDate = dayjs(booking.ngayDen);
  const bookingEndDate = dayjs(booking.ngayDi);
  const selectionStartDate = dayjs(selectionRange.startDate);
  const selectionEndDate = dayjs(selectionRange.endDate);

  const isBookingInRange = selectionStartDate.isSame(bookingStartDate, 'day') ||
                           selectionEndDate.isSame(bookingEndDate, 'day') ||
                           selectionStartDate.isBefore(bookingStartDate, 'day') ||
                           selectionEndDate.isAfter(bookingEndDate, 'day');

  return !isBookingInRange;
});

  const disabledRanges = disabledRangesFiltered.map((day) => {
