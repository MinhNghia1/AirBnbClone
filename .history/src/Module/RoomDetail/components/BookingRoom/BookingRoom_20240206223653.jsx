...
import { useParams } from "react-router-dom"; // Import thư viện react-router-dom
...
export default function BookingRoom({
  ...
  const { id } = useParams(); // Trích xuất ID phòng từ URL
  ...
  const { mutate } = useMutation({
    mutationFn: (value) => {
      const valueForm = {
        ...
        maPhong: id, // Sử dụng ID phòng từ URL
  ...
  const disabledRanges = ListOfRoomIsBooked.filter((booking) => booking.maPhong !== roomId).map((day) => {
    return { startDate: new Date(day.ngayDen), endDate: new Date(day.ngayDi) };
  });

  const isDateInDisabledRange = (date) => {
    return disabledRanges.some((disabledRange) => {
      return date >= disabledRange.startDate && date <= disabledRange.endDate;
    });
  };
