...
export default function ModalEditImage({ isOpen, onclose, onUpdate,locationId }) {
  ...
  const handleUploadImg = async (hinhAnh, locationId) => {
    try {
      setIsLoading(true);
      const resp = await editImg(hinhAnh, locationId);
      onclose(false);
      onUpdate(resp);
      if (resp) {
        Swal.fire({
  ...
  const handleUpload = (value) => {
    ...
    // Truyền id từ props khi gọi hàm handleUploadImg
