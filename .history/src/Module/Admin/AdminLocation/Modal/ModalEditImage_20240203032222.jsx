...
import { toast } from "your-toast-library"; // Import your preferred toast library
...
export default function ModalEditImage({ isOpen, onclose, locationId }) {
  ...
  const handleUploadImg = async (hinhAnh, Id) => {
    try {
      ...
      console.error(error);
      toastast.error("An unexpected error occurred."); // Add a toast message for unexpected errors
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUpload = async (value) => {
    ...
    if (locationId !== undefined) {
      await handleUploadImg(hinhAnh, locationId);
    } else {
